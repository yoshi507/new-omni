...
class AI(commands.Cog):
    ...
    async def _reply_ai(self, interaction: discord.Interaction, prompt: str):
        ...
        ok, text = await groq_client.chat(interaction.guild.id, prompt, user_id=interaction.user.id)
        if not ok:
    ...
    async def clearmemory(self, interaction: discord.Interaction):
        ...
        groq_client.clear_memory(interaction.guild.id, interaction.user.id)
