...
def _staff_role_ids(tcfg: dict) -> list[str]:
    ids: list[str] = []
    single = tcfg.get("staffRoleId")
    if single:
        ids.append(str(single).strip())
    multi = tcfg.get("staffRoleIds") or []
    if isinstance(multi, str):
        multi = [r.strip() for r in multi.split(",") if r.strip()]
    for r in multi:
        s = str(r).strip()
...
def _is_staff(member: discord.Member, tcfg: dict) -> bool:
    ...
    if member.guild_permissions.manage_messages:
        return True
    role_ids = set(_staff_role_ids(tcfg))
...
class Tickets(commands.Cog):
    ...
    async def setup(
        ...
        def mut(d):
            ...
            if staff_role:
                t["staffRoleId"] = str(staff_role.id)
    ...
    async def button_remove(self, interaction: discord.Interaction, button: str):
        ...
        new = [
            b
            for b in buttons
            if str(b.get("id", "")).lower() != target and str(b.get("label", "")).lower() != target
        ]
    ...
    async def _create_ticket(self, interaction: discord.Interaction, kind: str, subject: str) -> None:
        ...
        if tcfg.get("claimEnabled", True):
            close_view.add_item(
                discord.ui.Button(
                    label="Claim",
                    style=discord.ButtonStyle.success,
                    custom_id="omnibot:ticket:claim",
                    emoji="🙋",
                )
            )
        close_view.add_item(
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        if cid == "omnibot:ticket:claim":
            ...
            for k, rec in recs.items():
                if str(rec.get("channelId")) == str(interaction.channel.id):
                    ...
                    break
            if tid:
                def mut_claim(d, _tid=tid):
                    d.setdefault("ticketRecords", {}).setdefault(_tid, {})["claimedBy"] = str(interaction.user.id)
                storage.update_guild(interaction.guild.id, mut_claim)
            await interaction.response.send_message(f"🙋 Ticket claimed by {interaction.user.mention}.")
            return

        if cid == "omnibot:ticket:close":
            if not isinstance(interaction.user, discord.Member):
                return
            tcfg = _ticket_cfg(interaction.guild.id)
            data = storage.load_guild(interaction.guild.id)
        ...
        if cid.startswith("omnibot:ticket:btn:"):
            button_id = cid.split(":")[-1]
            data = storage.load_guild(interaction.guild.id)
            buttons = (data.get("tickets") or {}).get("buttons") or []
            ...
            if btn.get("useForm"):
                ...
                await self._create_ticket(
                    interaction,
                    kind=str(btn.get("purpose") or button_id),
                    subject=str(btn.get("label") or "Support"),
                )
