...
def create_app(bot, deploy_marker: str) -> FastAPI:
    ...
    async def post_ticket_panel(guild_id: str, body: TicketPanelBody, request: Request):
        ...
        if not tcfg.get("categoryId"):
            raise HTTPException(
                400,
                "Set a ticket category first (/ticket setup in Discord).",
            )
        ...
        if not buttons:
            raise HTTPException(
                400,
                "No ticket buttons configured. In Discord run /ticket button add, then try again.",
            )
        ...
        for i, btn in enumerate(buttons[:10]):
            view.add_item(
                _discord.ui.Button(
                    ...
                    style=style_map.get(str(btn.get("style") or "primary"), _discord.ButtonStyle.primary),
